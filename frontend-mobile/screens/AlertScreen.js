...
export default function AlertScreen() {
  ...
  return (
    ...
    >
      <EnableSwitch
        getEnabled={() => alertEnabled}
        setEnabled={ns => setAlertEnabled(ns)}
      />
...
function LocationDisplay({ getAlertEnabled }) {
  const [location, setLocation] = React.useState({
    lat: "Enable Alert to see location",
    lng: ""
  });
...
TaskManager.defineTask("location", ({ data: { locations }, error }) => {
  ...
  try {
    ...
    fetch(
      API_URL +
        "area-popularity/?lat=" +
        locations.reverse()[0]?.coords.latitude +
        "&lng=" +
        locations.reverse()[0]?.coords.longitude
    )
      .then(res => res.json())
      .then(res => {
        let crowd = res
          .map(a =>
            a.current_popularity
              ? Number(a.current_popularity)
              : a.expected_popularity
              ? Number(a.expected_popularity)
              : 0
          )
          .reduce((a, b) => a + b);
        prevCrowd = currCrowd;
      ...
      .then(ratio => {
        if (ratio >= 1.2) {
          Notifications.presentLocalNotificationAsync({
            title: "Crowd Alert!",
            body:
              "There has been a significant increase in the crowd in your area"
          });
        }
      });
  } catch (e) {
